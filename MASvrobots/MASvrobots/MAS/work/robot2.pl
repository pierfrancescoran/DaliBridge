...
main:-use_module(library(between)),use_module('mas_modules/utils.txt'),use_module('mas_modules/world_objects.txt'),retrieveIdentity,assert(time(-1)),nl,write('Started'),nl,joinWorld.

joinWorld:-nl,write('Joining world...'),nl,send(world,join).

eve(received(world,joined,var_SpawnX,var_SpawnY,var_WorldWidth,var_WorldHeight)):-write('Joined!'),nl,write('Spawned at ('),write(var_SpawnX),write(', '),write(var_SpawnY),write(')'),nl,write('World has width = '),write(var_WorldWidth),write(' and height = '),write(var_WorldHeight),nl,assert(worldWidth(var_WorldWidth)),assert(worldHeight(var_WorldHeight)),newStep.

newStep:-time(var_CurrentTime),retractall(time(var_X)),var_NewTime is var_CurrentTime+1,assert(time(var_NewTime)),identity(var_Agent),nl,write(var_Agent),write(': begin new step, time '),write(var_NewTime),nl,nl,write('Sensing environment...'),nl,send(world,sense).

eve(received(world,position,var_X,var_Y)):-nl,write('My position is ('),write(var_X),write(','),write(var_Y),write(')'),nl,time(var_Time),assert(position(var_X,var_Y,var_Time)).

eve(received(world,senseNewY)):-nl.

eve(received(world,cell,var_X,var_Y,var_Type)):-write(var_Type),time(var_Time),assert(cell(var_X,var_Y,var_Type,var_Time)).

eve(received(world,senseEnd)):-nl,nl,act,newStep.

act:-time(var_TimeNow),position(var_MyX,var_MyY,var_TimeNow),closest(var_EnemyX,var_EnemyY,enemy),distance(var_EnemyX,var_EnemyY,var_MyX,var_MyY,var_Distance),var_Distance is 1,write('trovato ('),write(var_EnemyX),write(','),write(var_EnemyY),write(')'),nl,send(world,attack,[var_EnemyX,var_EnemyY]).

act:-closest(var_EnemyX,var_EnemyY,enemy),write('Moving towards closest enemy at ('),write(var_EnemyX),write(','),write(var_EnemyY),write(')'),nl,moveTowards(var_EnemyX,var_EnemyY).

act:-closest(var_X,var_Y,void),write('Moving towards closest unknown cell at ('),write(var_X),write(','),write(var_Y),write(')'),nl,moveTowards(var_X,var_Y).

act:-write('Nothing doable, waiting...'),nl.

recallCell(var_X,var_Y,enemy):-time(var_TimeNow),position(var_MyX,var_MyY,var_TimeNow),worldObject(var_AgentCellType,agent),worldWidth(var_WorldWidth),worldHeight(var_WorldHeight),var_XUpper is var_WorldWidth-1,var_YUpper is var_WorldHeight-1,between(0,var_XUpper,var_X),between(0,var_YUpper,var_Y),recallCell(var_X,var_Y,var_AgentCellType),(var_X\=var_MyX;var_Y\=var_MyY).

recallCell(var_X,var_Y,void):-worldWidth(var_WorldWidth),worldHeight(var_WorldHeight),var_XUpper is var_WorldWidth-1,var_YUpper is var_WorldHeight-1,between(0,var_XUpper,var_X),between(0,var_YUpper,var_Y),voidCell(var_X,var_Y).

recallCell(var_X,var_Y,var_Type):-cell(var_X,var_Y,var_Type,var_Time),\+ (cell(var_X,var_Y,var__,var_OtherTime),var_OtherTime>var_Time).

voidCell(var_X,var_Y):- \+cell(var_X,var_Y,var__,var__).

closest(var_X,var_Y,var_CellType):-time(var_TimeNow),position(var_MyX,var_MyY,var_TimeNow),recallCell(var_X,var_Y,var_CellType),distance(var_X,var_Y,var_MyX,var_MyY,var_Distance),\+ (recallCell(var_OtherX,var_OtherY,var_CellType),distance(var_OtherX,var_OtherY,var_MyX,var_MyY,var_OtherDistance),var_OtherDistance<var_Distance).

distance(var_FromX,var_FromY,var_ToX,var_ToY,var_Distance):-var_Distance is max(abs(var_FromX-var_ToX),abs(var_FromY-var_ToY)).

moveTowards(var_X,var_Y):-time(var_TimeNow),position(var_MyX,var_MyY,var_TimeNow),var_XDistance is var_X-var_MyX,var_YDistance is var_Y-var_MyY,(var_XDistance>0,var_XDelta is 1;var_XDistance=0,var_XDelta is 0;var_XDistance<0,var_XDelta is-1),(var_YDistance>0,var_YDelta is 1;var_YDistance=0,var_YDelta is 0;var_YDistance<0,var_YDelta is-1),var_MyNewX is var_MyX+var_XDelta,var_MyNewY is var_MyY+var_YDelta,send(world,'move',[var_MyX,var_MyY,var_MyNewX,var_MyNewY]).

eve(received(world,dispose)):-write('My life ended, goodbye...'),nl,halt.

eve(received(var_From,var_Message)):-nl,write('Received '),write(var_Message),write(' from '),write(var_From),nl.
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_448301,var_Ontology,_448305),_448295),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_448339)),a(message(var_Ag,propose(var_A,[_448339],var_AgI))),retractall(ext_agent(var_Ag,_448377,var_Ontology,_448381)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_448175,var_Ontology,_448179),_448169),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_448245,var_Ontology,_448249)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_448063,var_Ontology,_448067),_448057),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_448119,var_Ontology,_448123)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_447627),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_447661),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_447443).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_447291),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_447239),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_447115,_447117)),clause(agente(_447137,_447139,_447141,var_S),_447133),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_446909,_446911,_446913,var_S),_446905),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_447009,[]),repeat,read(_447009,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_447009).
